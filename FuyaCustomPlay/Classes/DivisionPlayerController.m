//
//  DivisionPlayerController.m
//  Runner
//
//  Created by fuya on 15/12/23.
//

#import "DivisionPlayerController.h"
...
/// 预加载的模型数组
@property (nonatomic, strong) NSMutableArray<DivisionLoaderModel *> *preloadArr;
@property (nonatomic, assign) BOOL isAnimating;
@property (nonatomic, assign) UIInterfaceOrientation orientation;

@end

@implementation DivisionPlayerController
...
- (instancetype)initWithContainerView:(UIView *)containerView
{
    if (self = [super init])
    {
        DivisionAttributeManager *mgr = [[DivisionAttributeManager alloc] init];
...
- (void)playTheIndexPath:(NSIndexPath *)indexPath playable: (id<DivisionPlayable>)playable
{
    // 播放前，先停止所有的预加载任务
...
- (void)playWithPlayable: (id<DivisionPlayable>)playable
{
    _currentPlayable = playable;
...
- (void)preload: (id<DivisionPlayable>)resource
{
    ...
    {
        index += 1;
        id<DivisionPlayable> model = self.playableArray[i];
        DivisionLoaderModel *preModel = [self getPreloadModel: model.video_url];
        if (preModel) {
...
- (void)cancelAllPreload
{
    @synchronized (self.preloadArr) {
        ...
        [self.preloadArr enumerateObjectsUsingBlock:^(DivisionLoaderModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [obj.loader close];
        }];
...
- (DivisionLoaderModel *)getPreloadModel: (NSString *)urlStr
{
    if (!urlStr)
        return nil;
    // 判断是否已在队列中
    ...
    DivisionLoaderModel *preModel = [[DivisionLoaderModel alloc] initWithURL:urlStr loader:loader];
    return preModel;
}
...
- (void)processLoader
{
    @synchronized (self.preloadArr) {
        if (self.preloadArr.count == 0)
            return;
        DivisionLoaderModel *model = self.preloadArr.firstObject;
...
{
    @synchronized (self.preloadArr) {
        DivisionLoaderModel *target = nil;
        for (DivisionLoaderModel *model in self.preloadArr) {
...
- (NSMutableArray<DivisionLoaderModel *> *)preloadArr
{
    if (_preloadArr == nil)
    {
...
// MARK: - Setter
- (void)setPlayableArray:(NSArray<id<DivisionPlayable>> *)playableArray
{
    ...
    for (id<DivisionPlayable> model in subArr)
    {
        DivisionLoaderModel *preload = [self getPreloadModel:model.video_url];
        if (preload) {
            @synchronized (self.preloadArr) {
